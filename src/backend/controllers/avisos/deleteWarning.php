...
try {
    ...
    // Prepara a consulta
    $query = $db->prepare("DELETE FROM avisos WHERE id = ?");
    ...
    $query->bind_param('i', $warn_id);

    // Executar a consulta
    if ($query->execute()) {
        $response['success'] = true;
        $response['message'] = 'Aviso deletado com sucesso.';
    } else {
        $response['message'] = 'Erro ao deletar aviso: ' . $query->error;
    }
